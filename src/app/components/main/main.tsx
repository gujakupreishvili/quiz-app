      ...
      {!show ? (
        <>
          <div className="flex flex-col md:w-full lg:mr-[120px]">
            ...
            <p className="text-[40px] mt-[-13px] font-semibold text-[#313E51] dark:text-white w-[283px] md:text-[64px] md:w-[444px] md:mt-[-20px]">Frontend Quiz!</p>
        ...
        <div className="flex flex-col gap-[40px] md:w-full lg:flex-row lg:items-baseline">
          <div className="flex flex-col items-center justify-center md:items-start lg:h-[522px] lg:justify-between lg:mr-[60px]">
            ...
            <h3 className="w-[327px] text-[20px] font-medium text-[#313E51] dark:text-white md:text-[36px] md:w-[640px] md:text-left lg:w-[456px]">{kitxva?.question || 'Loading...'}</h3>
            </div>
            <div className="border-[0px]  w-[327px] h-[16px] p-[4px] bg-white dark:bg-[#3B4D66] rounded-[999px] mt-[24px] md:w-[614px] lg:mb-[170px]">
              <div className=" bg-[#A729F5]  h-[8px] rounded-[99px] mt-[-1px]" style={{ width: `${rect}%` }}>
            ...
            {kitxva && kitxva.answers ? (
              <>
                {Object.entries(kitxva.answers).map(([key, value]) => (
                  <button
                    key={key}
                    className={`flex  relative w-[327px] h-[64px] bg-white dark:bg-gray-800 dark:text-white p-[12px] gap-[16px] items-center rounded-[12px] group cursor-pointer focus:border-yellow-400 focus:border-[2px] md:w-[616px] md:h-[56px] md:p-[12px] lg:w-[524px] lg:h-[76px] lg:px-[20px] lg:py-[18px] lg:text-[28px] lg:rounded-[24px] 
                  ...
                  >
                    <div
                      ...
                      ${lock ? (ans === key && kitxva?.correct === key ? 'bg-green-400' : ans === key ? 'bg-red-400' : ans === key ? 'bg-red-400' : key === kitxva?.correct ? 'bg-green-400' :'') : ''}`}
                    >
                      {key}
                    </div>
                    {value}
                ...
                {!empty &&
                <>
                <p className="text-red-500 dark:text-white flex items-center justify-center gap-[8px] md:w-full"><span className="text-red-500 text-[24px]"><CiCircleRemove /></span>Please select an answer</p>
                </>
                }
